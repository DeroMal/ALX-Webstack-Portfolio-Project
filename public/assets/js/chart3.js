...
function fetchChartData() {
    fetch('/chart-data2').then(response => response.json()).then(data => {
...
function getTimeDiffString(diff) { var timeDiffString = ""; var days = diff.days(); var hours = diff.hours(); var minutes = diff.minutes(); var seconds = diff.seconds(); if (days > 0) { timeDiffString += days + " day" + (days > 1 ? "s " : " "); } if (hours > 0) { timeDiffString += hours + " hour" + (hours > 1 ? "s " : " "); } if (minutes > 0) { timeDiffString += minutes + " minute" + (minutes > 1 ? "s " : " "); } if (seconds > 0) { timeDiffString += seconds + " second" + (seconds > 1 ? "s " : " "); } if (timeDiffString === "") { timeDiffString = "just now"; } else { timeDiffString += "ago"; } return timeDiffString; }
